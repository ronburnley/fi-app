...
|---------|------|---------|
| v8.7 | 2026-02-21 | **Clickable FI Status Indicator** — FI Status Indicator in the header is now a clickable shortcut to the Results page (step 8). Always-visible "View full results →" action strip below the indicator when data is sufficient. Renders as `<button>` with full keyboard/screen-reader accessibility (`aria-label`, `focus-visible` outline). Hover brightens border glow, text turns white, arrow shifts right and turns green. Active press scales to 0.97. Data readiness gate (`hasMinimumDataForResults` in new `src/utils/dataReadiness.ts`) requires age > 0, at least one asset, and at least one expense. Not clickable on Results page or when data is insufficient. Added `returnStep: number \| null` to `WizardState` — tracks which step the user came from. `goToResults()` and `clearReturnStep()` on WizardContext. Back button on Results returns to the originating step (not step 7) when accessed via shortcut; normal step-through behavior unchanged. Zero calculation engine changes. 106 tests. |
