...
|---------|------|---------|
| v8.5 | 2026-02-18 | **Elegant Homepage + Reworked Welcome Step** *(feature/homepage branch)* — Added full marketing homepage (`HomePage.tsx`) with hero, "How It Works" 3-step section, and 4-card feature highlights. Session-gated: shown once per session (via `sessionStorage`), hidden after user enters app, reappears after sign-out. Auth-aware CTAs: authenticated users see "Welcome back, [name]" + "Continue to your plan →"; guests/unauthenticated see "Get Started — it's free" + Google sign-in. Removed "How it works" info callout from WelcomeStep; updated WelcomeStep heading to "Your Profile". Added ~200 lines of homepage CSS. 118 tests passing. |
